...
export function apply(ctx, config = {}) {
  ...
  const router = createSinglePassSemanticRouter({
    adapter: createDshLlmRoutingAdapter({ llm: ctx.llm, logger: ctx.logger, ...resolved }),
  ...
  const fiber = ctx.inject(["relayEvents"], scope => {
    ...
    scope.effect(() => scope.relayEvents.registerRouter(router), "relay semantic router provider");
  });
...
export function resolveConfig(config = {}, env = process.env) {
  ...
  return {
    provider,
    model,
    timeoutMs: positiveInteger(config.timeoutMs, 60_000),
